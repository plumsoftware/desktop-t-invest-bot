...
fun Register(navigator: Navigator) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect {
            when(it) {
                ...
                Effect.Next -> {
                    navigator.navigate(route = "${Route.Auth.AUTH}/true/${viewModel.state.value.name}/${viewModel.state.value.password}")
                }
            }
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            PrimaryTextInput(
                startText = viewModel.state.value.name,
...
private fun RegisterPreview() {
    AppTheme {
        Register(navigator = rememberNavigator())
    }
}